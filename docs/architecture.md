...
- [x] أول تشغيل حقيقي (benchmark) بأرقام V0 — **بوابة المرحلة مُغلقة 2026-09-17**:
  تشغيل LIVE على `config/icp/v0.yaml` (job `job-ce43751280`): 64 مرشح → 35 بعد
  dedup → 25 مؤهل → **5 leads نهائية** (24 للمراجعة)، contact coverage 40%،
  تكلفة $0. التقرير: `outputs/report.md`.
